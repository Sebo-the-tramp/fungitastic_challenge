...
def main() -> None:
    ...
    X_train = torch.cat([train_data["cls_tokens"],train_data["mean_pooled_patch_tokens"]], dim=1).to(device)
    X_val = torch.cat([val_data["cls_tokens"],val_data["mean_pooled_patch_tokens"]], dim=1).to(device)
    X_test = torch.cat([test_data["cls_tokens"],test_data["mean_pooled_patch_tokens"]], dim=1).to(device)

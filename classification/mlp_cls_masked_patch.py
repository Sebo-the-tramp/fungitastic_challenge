...
def main() -> None:
    ...
    X_train = torch.cat([train_data["cls_tokens"],train_data["mean_pooled_masked_patch_tokens"]], dim=1).to(device)
    X_val = torch.cat([val_data["cls_tokens"],val_data["mean_pooled_masked_patch_tokens"]], dim=1).to(device)
    X_test = torch.cat([test_data["cls_tokens"],test_data["mean_pooled_masked_patch_tokens"]], dim=1).to(device)

    y_train = train_data["labels"].to(device)
    y_val = val_data["labels"].to(device)
    y_test = test_data["labels"].to(device)

    full_train_data = torch.cat([X_train, X_val]).to(device)
    full_train_labels = torch.cat([y_train, y_val]).to(device)

...
def fractional_whiten_features(
    ...
    whitening_scale = scale.clamp_min(FRACTIONAL_EPS).pow(beta)
    train_features = train_centered @ basis / whitening_scale
    test_features = test_centered @ basis / whitening_scale
    return train_features, test_features


@torch.no_grad()
def prototype_method_fractional_white(
    train_features: torch.Tensor,
    ...
    train_features, test_features = fractional_whiten_features(
        train_features,
        test_features,
        fractional_dim=fractional_dim,
        beta=beta,
    )
    prototypes = build_prototypes(train_features, train_labels, num_classes)
...
def run_sweep(
    ...
    fractional_dim: int = FRACTIONAL_DIM,
    beta: float = FRACTIONAL_BETA,
    save_csv: bool = True,
    ...
    for seed in tqdm(seeds, desc="Seeds", position=0):
        ...
        for samples_per_class in tqdm(range(min_samples, max_samples + 1), desc=f"Samples seed={seed}", position=1, leave=False):
            ...
            raw_data, metrics_data = prototype_method_fractional_white(
...
if __name__ == "__main__":
    ...
    experiment_name = f"prototype_fractional_white_beta_{format_tag(FRACTIONAL_BETA)}_dim_{FRACTIONAL_DIM}"

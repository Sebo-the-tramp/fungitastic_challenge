...
def zca_whiten_features(
    ...
    zca_dim = min(zca_dim, train_features.shape[0] - 1, train_features.shape[1])
    assert zca_dim > 0
    ...
    _, singular_values, basis = torch.pca_lowrank(train_centered, q=zca_dim, center=False)
    basis = basis[:, :zca_dim]
    scale = singular_values[:zca_dim] / (train_features.shape[0] - 1) ** 0.5
    inv_scale = scale.clamp_min(ZCA_EPS).reciprocal()
    train_features = (train_centered @ basis * inv_scale) @ basis.T
    test_features = (test_centered @ basis * inv_scale) @ basis.T
    return train_features, test_features


@torch.no_grad()
def prototype_method_zca(
    train_features: torch.Tensor,
    train_labels: torch.Tensor,
    test_features: torch.Tensor,
    test_labels: torch.Tensor,
    ...
    train_features, test_features = zca_whiten_features(train_features, test_features, zca_dim=zca_dim)
...
def run_sweep(min_samples=1, max_samples=None, seeds=[], experiment_name="", masks={}, zca_dim=128, save_csv=True):
    all_results = []
    ...
    for seed in tqdm(seeds, desc="Seeds", position=0):
        ...
        for samples_per_class in tqdm(
            ...
            raw_data, metrics_data = prototype_method_zca(
...
if __name__ == "__main__":
    ...
    print(num_seeds)
    experiment_name = f"prototype_zca_white_{zca_dim}"
    ...
    results = run_sweep(1, max_samples, seeds=num_seeds, experiment_name=experiment_name, masks=masks, zca_dim=zca_dim, save_csv=True)

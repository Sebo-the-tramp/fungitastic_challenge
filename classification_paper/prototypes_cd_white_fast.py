...
def cholesky_whiten_features(
    train_features: torch.Tensor,
    test_features: torch.Tensor,
    cholesky_eps: float = CHOLESKY_EPS,
    ...
    covariance = train_centered.T @ train_centered / (train_features.shape[0] - 1)
    covariance = covariance + cholesky_eps * torch.eye(
        covariance.shape[0],
        device=covariance.device,
        dtype=covariance.dtype,
    )
    cholesky = torch.linalg.cholesky(covariance)
    train_features = torch.linalg.solve_triangular(cholesky, train_centered.T, upper=False).T
    test_features = torch.linalg.solve_triangular(cholesky, test_centered.T, upper=False).T
    return train_features, test_features


@torch.no_grad()
def prototype_method_cd(
    train_features: torch.Tensor,
    train_labels: torch.Tensor,
    test_features: torch.Tensor,
    test_labels: torch.Tensor,
    ...
    train_features, test_features = cholesky_whiten_features(
        train_features,
        test_features,
...
def run_sweep(min_samples=1, max_samples=None, seeds=[], experiment_name="", masks={}, cholesky_eps: float = CHOLESKY_EPS, save_csv=True):
    all_results = []
    ...
    for seed in tqdm(seeds, desc="Seeds", position=0):
        ...
        for samples_per_class in tqdm(
            ...
            raw_data, metrics_data = prototype_method_cd(
...
if __name__ == "__main__":
    ...
    print(num_seeds)
    experiment_name = f"prototype_cd_white_eps_{cholesky_eps:g}"

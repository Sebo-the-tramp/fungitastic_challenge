...
if BACKBONE == "dinov3-vit7b16-pretrain-lvd1689m":
    CLASSIFICATION_RESULTS_DIR = Path(__file__).resolve().parent / "results"
else:
    CLASSIFICATION_RESULTS_DIR = Path(__file__).resolve().parent / f"results_{BACKBONE}"

print("USING", CLASSIFICATION_RESULTS_DIR)
...
def run_sweep(min_samples=1, max_samples=None, seeds=[], experiment_name="", masks={}, pca_dim=128, save_csv=True):
    ...
    for seed in tqdm(seeds, desc="Seeds", position=0):
        ...
        seed_results = []
        csv_path_prefix = f"{CLASSIFICATION_RESULTS_DIR}/{experiment_name}/{seed}"
...
if __name__ == "__main__":
    ...
    print(num_seeds)
    experiment_name = f"prototype_pca_white_cosine_{pca_dim}"

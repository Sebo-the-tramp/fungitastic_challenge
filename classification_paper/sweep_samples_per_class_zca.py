...
def run_sweep(min_samples=1, max_samples=None, save_csv=True):
    ...
    for samples_per_class in range(min_samples, max_samples + 1):
        ...
        # Whitening: divide by sqrt(explained_variance_)
        train_pca = pca.transform(train_features) / np.sqrt(pca.explained_variance_)
        val_pca = pca.transform(val_features) / np.sqrt(pca.explained_variance_)
        test_pca = pca.transform(test_features) / np.sqrt(pca.explained_variance_)
        train_data_balanced['cls_tokens'] = torch.tensor(train_pca).float()
        val_data_balanced['cls_tokens'] = torch.tensor(val_pca).float()
        test_data_balanced['cls_tokens'] = torch.tensor(test_pca).float()
        ...
        plot_sweep(results, save_only=True, filename='sweep_samples_per_class_pca_whiten_plot.png')
    return results
...
def plot_sweep(results, save_only=False, filename='sweep_samples_per_class_pca_plot.png'):
    ...
    plt.plot(x, [r['accuracy_euclidean'] for r in results], label='Euclidean (PCA-whiten)')
    plt.plot(x, [r['accuracy_cosine'] for r in results], label='Cosine (PCA-whiten)')
    plt.plot(x, [r['accuracy_mahalanobis'] for r in results], label='Mahalanobis (PCA-whiten)')
    plt.xlabel('Samples per Class')
    plt.ylabel('Accuracy')
    plt.title('Accuracy vs Samples per Class (PCA-whiten)')
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    plot_sweep(results, filename='sweep_samples_per_class_pca_whiten_plot.png')
